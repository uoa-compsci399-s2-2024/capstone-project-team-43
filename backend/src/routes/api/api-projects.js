...
router.post("/", async (req, res) => {
    const { title, description, owner_id, special_requirements, available_resources, preferred_skills, project_deliverable, created, expiry, status, max_teams, project_number, semester_id } = req.body;

    if (!title || !description || !owner_id || !project_deliverable || !created || !expiry || !status || !max_teams || !project_number || !semester_id) {
