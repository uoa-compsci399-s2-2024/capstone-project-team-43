...
router.post("/", async(req, res) => {
    const { start_date, end_date, semester_name } = req.body;
    if (!start_date || !end_date || !semester_name) {
        return res.status(422);
    }
    
    //Details are valid and now passed to createSemester function to query into database
    const semester = await createSemester(start_date, end_date, semester_name);
